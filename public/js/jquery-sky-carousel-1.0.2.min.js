...
SKY.CarouselItem.createReflection=function(a,b,c){var d=a.width(),g=a.height(),e=null;SKY.Utils.hasCanvasSupport()?(e=$jload("<canvas>"),ctx=e.get(0).getContext("2d"),e.attr({width:d,height:b}),e.addClass("reflection"),ctx.save(),ctx.translate(0,g),ctx.scale(1,-1),ctx.drawImage(a.get(0),0,0,d,g),ctx.restore(),ctx.globalCompositeOperation="destination-out",a=ctx.createLinearGradient(0,0,0,b),a.addColorStop(0,"rgba(0, 0, 0, "+(1-c)+")"),a.addColorStop(1,"rgba(0, 0, 0, 1.0)"),ctx.fillStyle=a,ctx.fillRect(0,
0,d,b)):(e=$jload("<img>"),e.attr({src:a.get(0).src}),e.css("filter",SKY.CarouselItem.getAlphaFilterStatement(c,b,g)));return e};SKY.CarouselItem.getAlphaFilterStatement=function(a,b,c){return"flipv progid:DXImageTransform.Microsoft.Alpha(opacity="+100*a+", style=1, finishOpacity=0, startx=0, starty=0, finishx=0, finishy="+100*(b/c)+")"};SKY.Container=function(a,b){this.element=a;this.carousel=b;this.x=0};
...
reflectionVisible:!1,reflectionDistance:4,reflectionSize:100,reflectionAlpha:0.38,slideSpeed:0.45,selectByClick:!1};b&&$jload.extend(this.settings,b);this.targetLeft=0;this.dragging=this.mouseOver=!1;this.extraDistanceUnit=this.scaleUnit=this.alphaUnit=this.centerY=this.centerX=this.timer=this.preloader=this.contentContainer=this.container=this.closestItem=this.selectedItem=null;this.carouselItems=[];this.dom={carousel:a};this.events={};this.init()};
SKY.Carousel.prototype={init:function(){this.initDOM();this.initConfigParams();this.initEvents();this.initContentWrapper();this.initContainer();this.initGradientOverlays();this.initNavigationButtons();this.initResizeListener();this.initKeyboardNavigation();this.initMouseWheelSupport();this.initAutoSlideshow();this.calculateUnits();this.update();this.dom.carousel.css("visibility","visible")},initDOM:function(){var a=this.settings;this.dom.document=$jload(document);this.dom.wrapper=this.dom.carousel.children(".sky-carousel-wrapper");
this.dom.container=this.dom.wrapper.children(".sky-carousel-container");this.dom.items=this.dom.container.children("li");this.dom.links=this.dom.container.find("li > a");this.dom.images=this.dom.container.find("li img");this.dom.carousel.addClass("sc-no-select");a.preload&&!1!=$jload.support.leadingWhitespace&&(this.dom.wrapper.css({visibility:"hidden",opacity:0}),a.showPreloader&&(this.preloader=$jload('<div class="sc-preloader"></div>'),this.dom.carousel.append(this.preloader)));this.dom.images.each(function(){$jload(this).addClass("sc-image");
...
(this.events.startEvent="touchstart.sc",this.events.moveEvent="touchmove.sc",this.events.endEvent="touchend.sc",this.events.touchCancel="touchcancel.sc"):(this.events.startEvent="mousedown.sc",this.events.moveEvent="mousemove.sc",this.events.endEvent="mouseup.sc")},initContainer:function(){var a=this,b=0;this.container=new SKY.Container(this.dom.container,this);this.dom.items.each(function(c){c=new SKY.CarouselItem($jload(this),a);c.subscribe("load",function(c){++b;if(b==a.dom.items.length)a.onAllLoaded()});
...
a.gradientOverlayColor,a.gradientOverlaySize),a=this.createGradientOverlay("right",a.gradientStartPoint,a.gradientEndPoint,a.gradientOverlayColor,a.gradientOverlaySize);this.dom.carousel.append(b);this.dom.carousel.append(a)}},initContentWrapper:function(){var a=$jload('<div class="sc-content-wrapper"></div>');this.contentContainer=$jload('<div class="sc-content-container"></div>');this.contentContainer.append('<div class="sc-content"><h2></h2><p></p></div>');a.append(this.contentContainer);!1!=$jload.support.leadingWhitespace&&
this.settings.preload&&this.contentContainer.css({visibility:"hidden",opacity:0});this.dom.carousel.append(a)},initNavigationButtons:function(){var a=this;if(this.settings.navigationButtonsVisible){var b=$jload('<a href="#" class="sc-nav-button sc-prev sc-no-select"></a>'),c=$jload('<a href="#" class="sc-nav-button sc-next sc-no-select"></a>');this.dom.carousel.append(b);this.dom.carousel.append(c);b.on("click",function(b){b.preventDefault();a.selectPrevious(a.settings.slideSpeed)});c.on("click",function(b){b.preventDefault();
a.selectNext(a.settings.slideSpeed)})}},initResizeListener:function(){var a=this;$jload(window).on("resize",function(b){a.updatePlugin(b)})},initKeyboardNavigation:function(){var a=this;s=this.settings;s.enableKeyboard&&this.dom.document.keydown(function(b){if("textarea"!=b.target.type&&"text"!=b.target.type)switch(b.keyCode){case 37:a.selectPrevious(s.slideSpeed);break;case 39:a.selectNext(s.slideSpeed)}})},initMouseWheelSupport:function(){var a=this,b=this.settings,c=this.dom.carousel.get(0);if(b.enableMouseWheel){var d=
